...
- `shirube gate quality --auto-fix` で PASS 確定（dogfooding）
- `shirube gate release` で SHIP 判定
...
npx @watchout/shirube@1.2.0 init
npx @watchout/shirube@1.2.0 init-feature hello
ls docs/{spec,impl,verify,ops}/hello.md  # 4ファイル存在確認
npx @watchout/shirube@1.2.0 trace verify         # PASS 確認

# 既存プロジェクトでマイグレーション動作確認
cd ~/Developer/haishin-puls-hub  # 本番適用後の確認
npx @watchout/shirube@1.2.0 migrate-to-v1.2 --dry-run
# → 出力内容を目視確認、問題なければ実運用
```
...
- `shirube trace verify` が既存 v1.1 プロジェクト（`docs_layers.enabled=false` または未設定）で誤って BLOCK する
- `shirube migrate-to-v1.2` で既存ファイルを破壊する報告
- セキュリティ脆弱性の報告
- `shirube gate release` を再実行して SHIP 判定が取れない

### 2.2 手順

```bash
# 1. npm で deprecate
npm deprecate @watchout/shirube@1.2.0 "Rolled back due to <reason>"

# 2. 前バージョンを latest タグに戻す
npm dist-tag add @watchout/shirube@1.1.x latest
...
|---|---|---|---|
| `shirube init-feature` 成功率 | 99.9% | 直近 100 実行のログ | 月 0.1% |
| `shirube trace verify` 実行時間 p95 | 5 秒以下（100 feature規模） | ベンチスイート | 月 5 回超過まで |
| `shirube migrate-to-v1.2` 実行時間 p95 | 30 秒以下（100 feature規模） | ベンチスイート | 月 3 回超過まで |
...
- **一次対応:**
  ...
  3. 意図して true の場合 → マイグレーションツール未実行の可能性。`shirube migrate-to-v1.2 --dry-run` で事前確認、問題なければ実行
