...
`04c` 準拠の matrix を持つ feature は、Gate 3 (IMPL.md format validation, FR-L4.3) で **「per-FR ≥1 件の `[検証済]` 」rule** を deterministic に check 可能 [文献確認: 親 SPEC FR-L6.3]。`[推測]` のみで構成された FR 行は WARNING 発行。
...
- Generation chain: `docs/specs/02_GENERATION_CHAIN.md` Step 3.45 (Lead IMPL Authoring) + Gate 3 (IMPL.md format validation, Sub-PR 0.2 で land 予定)
...
[文献確認: relative path link は 2026-05-02 時点で全て有効、ただし Sub-PR 0.2 (Step 3.45 + Gate 3) 未 land 時は該当 anchor が存在しない可能性あり — Sub-PR 0.2 land 後に anchor 整合]
