...
> Format spec: [04b_IMPL_FORMAT.md](04b_IMPL_FORMAT.md)

Step 3 (Technical / SSOT freeze) 完了後、lead-bot が [04b_IMPL_FORMAT.md](04b_IMPL_FORMAT.md) (§1〜§10 必須 + evidence label 規約) に従い per-feature `IMPL.md` (施工図) を起草する。詳細な workflow / input / output / exit criteria は親 SPEC `lead-impl-workflow/SPEC.md` で定義。
...
| §1〜§10 header (FR-L4.2) | [04b_IMPL_FORMAT.md](04b_IMPL_FORMAT.md) 必須セクション header (`## §1` 〜 `## §10`) が存在 | WARNING |
...
両者は直交。Gate 2 は Pre-impl gate より前に CI で走り、IMPL.md 不在 / format 不備を機械的に捕捉する第 0 段。Feasibility PoC + per-FR traceability matrix は [04c_FEASIBILITY_POC_FORMAT.md](04c_FEASIBILITY_POC_FORMAT.md) と parent SPEC FR-L6 で扱う。

## Step 3.45 Reference (Feasibility PoC)

> **Owner**: lead-bot (PoC authoring) + codex-auditor (validation)
> Parent SPEC: `lead-impl-workflow/SPEC.md` FR-L6
> Feasibility format spec: [04c_FEASIBILITY_POC_FORMAT.md](04c_FEASIBILITY_POC_FORMAT.md)
> Related IMPL format spec: [04b_IMPL_FORMAT.md](04b_IMPL_FORMAT.md)

Step 3.4 の [04b_IMPL_FORMAT.md](04b_IMPL_FORMAT.md) で `[推測]` として残した assertion は、Step 3.45 の [04c_FEASIBILITY_POC_FORMAT.md](04c_FEASIBILITY_POC_FORMAT.md) に従う PoC で検証し、実装着手前に `[検証済]` へ昇格する根拠を残す。
