...
- サイクル1: 全レビュアーが確認。BLOCKER/CRITICALのみ修正要求。WARNING以下はfollow-up issueに記録
- サイクル2: BLOCKER/CRITICALを指摘したレビュアーが修正diffのみ確認。
             新しい指摘は追加しない（スコープ固定）。
             **レビュアーチェーン再走は不要。** Layer 0のCI greenで自動検証は完了。
             合意済み修正の確認は指摘者1名で十分。
- サイクル3: サイクル2で解決しなければエスカレーション
