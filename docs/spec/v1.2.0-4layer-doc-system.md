...
# SPEC: ADF v1.2.0 — 4 層ドキュメント体系導入

> Honesty labels: [検証済] / [文献確認] / [推測] を主要 claim に付与する。
> route: `route:ceo-approval` (= ADF base spec、v1.2.X 系列 foundation)

## 0. メタ

- 作成日: 2026-04-20 (gdrive 原本)、repo landing: 2026-05-13 (ARC reshape)
- ID: **SPEC-DOC4L-001**
- 起点 proposal: `proposals/v1.2.x/adf-v1.2.0-spec-proposal.md` (gdrive `IYASAKA/開発/ADF/v1.2.0_2026-04-20/specs/SPEC.md` verbatim copy)
- 関連 ARC handoff: `proposals/v1.2.x/adf-v1.2.0-arc-handoff.md` (CEO 起草、ARC への引き渡し指示)
- 関連 ADR: ADR-017 (4層ドキュメント体系の採用判断)
- 対応リポジトリ: `~/Developer/ai-dev-framework`
- 対応バージョン: ADF v1.2.0
- 引き渡し: ARC (CEO directive 2026-04-20 per、CEO `8ad11b2a` 2026-05-13 「進めて」 GO)
- 落とし込み chain: ARC reshape → adf-lead L1 → auditor L2 → CTO L3 → CEO L4 → merge

### 0.1 本 spec の位置付け (ARC reshape 注記、2026-05-13 追加)

本 SPEC-DOC4L-001 は ADF v1.2.0 の **4 層ドキュメント体系本体定義**。v1.2.1 (SPEC-DOC4L-008、Claude Code Hooks)、v1.2.2 (SPEC-DOC4L-009)、v1.2.3 (SPEC-DOC4L-010-011)、v1.2.4 (SPEC-DOC4L-012-015)、v1.2.5 (SPEC-DOC4L-016)、v1.2.6 (SPEC-DOC4L-017) は本 spec を **base** として参照する v1.2.X 系列の上位構築。

repo landing 経緯:
- 2026-04-20 gdrive で SPEC.md / IMPL.md / VERIFY.md / OPS.md の 4 file が起草
- 2026-04-XX 〜 2026-05-12 期間で v1.2.0 の **実装 code は merge** (PR #96 step 1/5 / PR #103 step 2/5 / PR #104 step 3/5) されたが、**spec doc 本体は repo に未 landing** 状態が継続
- 2026-05-13 ARC が gdrive verbatim copy → repo reshape で landing 復旧

本 spec が landing されたことで v1.2.X 系列が依拠する **base が repo 内に揃う** = ADF v3.2 README と v1.2.X spec の bridge 完成 first step。

### 0.2 gdrive 原本との関係

- 原本 path: `IYASAKA/開発/ADF/v1.2.0_2026-04-20/specs/SPEC.md` (271 行)
- 本 file は **gdrive 原本 verbatim 内容 + repo template frontmatter + §0 メタ 拡張**
- 本文 (§1 以降) は **verbatim**、追加なし、削除なし
- proposal verbatim copy は `proposals/v1.2.x/adf-v1.2.0-spec-proposal.md` で確認可能
...
既存 v1.1.x プロジェクトが v1.2 に踏み切れるように、`shirube migrate-to-v1.2` を v1.2.0 必須スコープとする。既存 SSOT を正規表現でスキャンし、feature 単位の 4 層雛形を自動生成する**純粋スクリプト**。雛形の中身（Gherkin シナリオ、型定義、SLO 数値等）は人間が記入。
...
shirube migrate-to-v1.2                # v1.1→v1.2 マイグレーション（純粋スクリプト）
```
...
- `shirube migrate-to-v1.2` は 100 feature 規模で 30 秒以内
...
- `shirube migrate-to-v1.2` 実行結果を `.framework/reports/migration-YYYYMMDD.md`（生成ファイル一覧、スキップ理由）
...
When `shirube migrate-to-v1.2` を実行
Then 終了コード 0
...
## §Design rationale: filesystem port 化非適用 (PR-1b〜1e 共通)

> ARC 判断 (2026-05-14、escalate `b98d336d` 応答 msg `6b01a233` per、auditor pre-impl gate [5][6] 衝突解消)。

template-generator / migration-engine / CLI commands の filesystem 操作は Node.js std lib (`fs`/`path`) 直接利用とし、port/adapter 抽象を導入しない。理由:

1. Node.js `fs` は runtime-guaranteed std lib であり、「外部世界 N 個」 (DB / API / queue / cloud storage 等) に該当しない。adapter pattern の本来用途 (multi-impl swap / cross-runtime portability) と無関係。
2. PR-1b 〜 PR-1e は「local project dir に markdown 雛形を生成する」leaf utility であり、変動軸 (template source / sink type) が単一固定。port が抽出すべき可変次元が存在しない。
3. Testing isolation は `os.tmpdir()` + `fs.mkdtemp` で達成可能、DI port を要しない。
4. PR-1a (parseSsot, merged #145) も fs 直接利用、本 PR 群で port 化すると既存 codebase との整合を破壊する。
5. 将来 cloud-fs / virtual-fs 要件が発生した時点で adapter 化を検討する (YAGNI)。

本 rationale により PR-1b〜1e auditor pre-impl gate [5] 変動軸分離 / [6] adapter 対称性 は **N/A 宣言** とする (= adapter pattern misapplication 回避)。
