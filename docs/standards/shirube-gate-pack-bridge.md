...
When a handoff, profile, or explicit `review_plan_ref` requires review planning, `run-rapid-lite-report` builds/checks `shirube-review-plan/v1` and writes `review-plan.json`. The review plan is the source of authority for standard independent structured audit and additional protected reviews. Freeform text such as `cto_review_question` is explanatory only and must not create or remove a review requirement.
...
When the review plan requires additional CTO, security, legal, privacy, or technical-owner review, owner exact-head approval remains forbidden until the additional review evidence is complete for the same exact head. The next action is `request_required_additional_review` and early owner approval blocks with `REVIEW-SEQ-001`.
