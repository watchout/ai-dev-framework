...
# VERIFY: ADF v1.2.0 — 4 層ドキュメント体系導入
...
- 起点 proposal: `proposals/v1.2.x/adf-v1.2.0-verify-proposal.md` (gdrive `VERIFY.md` verbatim copy)
- 対応 SPEC: SPEC-DOC4L-001、対応 IMPL: IMPL-DOC4L-001
- 引き渡し: ARC (CEO `8ad11b2a` per)

### 0.1 gdrive 原本との関係

- 原本 path: `IYASAKA/開発/ADF/v1.2.0_2026-04-20/specs/VERIFY.md` (286 行)
...
```gherkin
Feature: shirube migrate-to-v1.2
  ...
  Scenario: dry-run で事前確認
    Given 同じプロジェクト状態
    When `shirube migrate-to-v1.2 --dry-run` を実行
    Then 終了コード 0
    And ファイルは生成されない
