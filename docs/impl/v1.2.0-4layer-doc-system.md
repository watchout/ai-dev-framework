...
- 起点 proposal: `proposals/v1.2.x/adf-v1.2.0-impl-proposal.md` (gdrive `IYASAKA/開発/ADF/v1.2.0_2026-04-20/specs/IMPL.md` verbatim copy)
- 対応 SPEC: SPEC-DOC4L-001 (`docs/spec/v1.2.0-4layer-doc-system.md`)
- 関連 ADR: ADR-017
- 引き渡し: ARC (CEO `8ad11b2a` 2026-05-13 「進めて」 GO)

### 0.1 本 IMPL の位置付け (ARC reshape 注記、2026-05-13 追加)

ADF v1.2.0 の実装は **PR #96 step 1/5 + PR #103 step 2/5 + PR #104 step 3/5** で既 merged (= code 反映済)。本 IMPL doc は **doc layer の retroactive landing** = code は既稼働、doc は今回 landing で 4 層 doc trace 完成。

### 0.2 gdrive 原本との関係

- 原本 path: `IYASAKA/開発/ADF/v1.2.0_2026-04-20/specs/IMPL.md` (623 行)
...
shirube migrate-to-v1.2 [--dry-run] [--ssot <path>]
```
...
### 3.3 `shirube migrate-to-v1.2` の正常系
...
- `shirube migrate-to-v1.2` は排他実行（`.framework/locks/migration.lock`）
