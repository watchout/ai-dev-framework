...
> このプロセスは `framework-feature-spec` Skill で自動実行できます。
> Skill には Deliberation Protocol（多視点対話）が内蔵されており、
> SSOT 生成前にセキュリティ×QA×エンジニアの3人が議論してレビューします。
>
> 詳細: templates/skills/feature-spec/SKILL.md
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  ⑦ 確認点を全て漏れなくリストし、ヒアリング
     （推奨パターン・モデルケースを必ず提示）
        │
        ▼
  ⑦.5 💬 Deliberation（多視点レビュー）
     （セキュリティ×QA×エンジニアの3人が議論）
     → ヒアリング結果の見落とし・矛盾を検出
     → 結果をユーザーに報告
        │
        ▼
...
Phase 3: 統合
  → 全員が合意した指摘事項をリスト化
  → 重要度（Critical / Major / Minor）を付与

Phase 4: ユーザー報告
  → Critical / Major の指摘をユーザーに報告
  → 対応方針を確認してから ⑧ に進む

※ 詳細プロトコル: templates/skills/_deliberation/DELIBERATION_PROTOCOL.md
```
...
## ⑧.5 🔒 Example-driven 必須セクション生成（Gate）

### 目的

```
SSOT のフォーマット（⑧）に落とし込んだ後、
§3-E/F/G/H の Example-driven セクションを
確実に埋める強制ステップ。

根拠:
  12_SSOT_FORMAT.md の「Example-driven 仕様（必須セクション）」で
  §3-E/F/G/H は MUST と定義されている。
  しかし過去の適用実績では、この4セクションが
  全ファイルでほぼ全滅する構造的問題が発生した。

  原因: テンプレートに含まれていても、
        フローに明示的な生成ステップがないと LLM がスキップする。

  対策: フローに明示的な Gate を設け、
        4セクション全てが埋まらないと次に進めない構造にする。
```

### 生成手順

```
AIは以下の順序で §3-E/F/G/H を生成する:

Step 1: §3-G 例外応答（Exception Response）を先に生成
────────────────────────────────────────────
  §5 API仕様のエラーレスポンスと
  §9 エラーハンドリングから、
  全例外条件を抜き出し、表に整理する。

  チェック: 例外条件が §9 のエラーケースと 1:1 で対応しているか

Step 2: §3-F 境界値（Boundary Values）を生成
────────────────────────────────────────────
  §4 データ仕様の全入力項目について、
  最小値/最大値/空/NULL/不正形式 の5パターンを定義する。

  チェック: §4.1 データ項目一覧の全項目が網羅されているか

Step 3: §3-E 入出力例（Example Table）を生成
────────────────────────────────────────────
  §3 の MUST 要件ごとに、具体的な入出力例を
  最低5ケース（正常系2 + 異常系3）作成する。
  Step 1, 2 の結果を活用して異常系・境界値ケースを含める。

  チェック: 5ケース以上あるか、正常系と異常系が含まれているか

Step 4: §3-H Gherkin テスト（Acceptance Tests）を生成
────────────────────────────────────────────
  §3 の全 MUST 要件に対して Gherkin Scenario を記述する。
  Step 3 の入出力例をシナリオ化する。

  チェック: 全 MUST 要件に対応する Scenario があるか
```

### Gate 条件（⑨ に進むための必須条件）

```
以下が全て満たされない限り、⑨ に進んではならない:

  □ §3-E: 入出力例が 5ケース以上（正常系2 + 異常系3 以上）
  □ §3-F: §4.1 の全データ項目に境界値が定義済み
  □ §3-G: §9 の全エラーケースに例外応答が定義済み
  □ §3-H: §3 の全 MUST 要件に Gherkin Scenario が存在

  1つでも ☐ なら: ⑨ に進まず、不足箇所を補完する
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 🔒 Example-driven セクション完全性チェック
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

■ §3-E 入出力例:    ✅ 7ケース（正常3 + 異常4）
■ §3-F 境界値:      ✅ 5項目 × 5パターン = 25定義
■ §3-G 例外応答:    ✅ 6例外条件 → 6応答定義
■ §3-H Gherkin:     ✅ 4 MUST要件 → 8 Scenario
...
|------|---------|-------|
| | 初版作成 | |
| | ⑦.5 Deliberation（多視点レビュー）を追加。Agent Skill 参照を追加 | || | ⑧.5 Example-driven 必須セクション生成（Gate）を追加。§3-E/F/G/H 全滅問題への構造的防止策 | |