...
## 11. Agent Skills（擬似マルチエージェント + 多視点対話）

### 概要

```
Agent Skills は、各フェーズを専門化した「AI の業務マニュアル」。
Claude.ai / Claude Code / Cursor の全環境で利用できる。

1つの LLM が Skills を切り替えることで、
複数の専門家チームが議論しながら開発を進める効果が得られる。
```
...
Agent Teams（§8）:
  - Claude Code CLI 専用
  - メインエージェントがサブエージェントを呼び出す
  - コンテキスト節約が主目的

Agent Skills（§11）:
  - Claude.ai / Claude Code / Cursor の全環境で利用可能
  - 各 Skill が独立した専門家として振る舞う
  - 品質向上・ステップ飛ばし防止が主目的
  - Deliberation Protocol で複数専門家が対話
```
...
```
templates/skills/（フレームワーク側）
├── SKILLS_INDEX.md              ← 全体像と利用方法
├── _deliberation/
│   └── DELIBERATION_PROTOCOL.md ← 多視点対話の共通プロトコル
├── orchestrator/SKILL.md
├── review-council/SKILL.md
├── discovery/SKILL.md
├── business-design/SKILL.md
...
プロジェクト側（setup-project.sh でデプロイ）:
.claude/skills/（上記と同じ構造）
```

### 詳細

```
各 Skill の詳細: templates/skills/SKILLS_INDEX.md
Deliberation の仕組み: templates/skills/_deliberation/DELIBERATION_PROTOCOL.md
```
...
| | §11 Agent Skills（擬似マルチエージェント + Deliberation Protocol）を追加 | |