...
## 10. Gate Engine Concepts (v1.2.0)

> **Added in v1.2.0** (issue #197/#198: public-grade Gate Engine baseline)
> Full spec: `docs/spec/v1.2.0-gate-engine.md`

### Goal Contract

A Goal Contract is required before a project can claim V0/V1 direction or strict/public-grade readiness. It must define:

- target version (`V0`, `V1`, or an explicit release goal)
...
An AI Change Record (ACR) is the structured evidence package for an AI-generated or AI-assisted PR. It must include:

- intent and task reference
- Goal Contract and plan references
- Context Pack id/hash
- agent/provider identity and permission mode
- allowed tools, denied tools, MCP servers/tools, sandbox/network posture
...
Seven diff-surface categories automatically escalate the tier to `full` regardless of declared tier:

| Category | Trigger patterns |
|----------|-----------------|
| `auth` | session, token, password, credential, login, JWT, OAuth, permission, role, RBAC |
| `db` | DROP TABLE, ALTER TABLE, migration, schema, database |
| `public-api` | REST endpoint, route, GraphQL, OpenAPI |
| `routing` | router, middleware, proxy |
| `runtime` | Dockerfile, docker-compose, k8s, Kubernetes, deploy.yml |
| `deploy` | deploy, CI/CD, release workflow |
| `governance` | merge authority, governance bone, audit |

See `src/cli/lib/protected-pattern-detector.ts` and `specs/07_AI_PROTOCOL.md §0`.

### Merge Authority

Merge authority is evaluated against declared tier, CI Gate 0 result, and protected-category detection. Only a PR with all three conditions satisfied can be auto-merged in strict profile:

1. CI Gate 0 passed (typecheck + lint + test + coverage + protected-pattern scan on exact HEAD SHA)
2. Declared tier ≥ required tier (or auto-promoted via protected-category detection)
3. Human review evidence attached (L1 + L2 LGTM for standard/full tier)

See `src/cli/lib/merge-authority-engine.ts` and `docs/spec/v1.2.0-merge-authority-gate.md`.

### v1.2.0 Change Log

| Date | Change |
|------|--------|
| 2026-05-23 | v1.2.0 Gate Engine — §10 Goal Contract, Context Pack, AI Change Record, Tier Auto-Promotion, Merge Authority concepts added (issue #329) |
