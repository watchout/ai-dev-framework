...
## Pre-Implementation Readiness Check（実装前の軽量事前監査）

```
目的:
  本監査（100点満点スコアリング）の前に、
  §3-E/F/G/H の存在を確認する軽量チェック。
  実装開始の Gate として機能する。

タイミング:
  - 11_FEATURE_SPEC_FLOW.md ⑧.5 完了時
  - 実装依頼を受けた時（CLAUDE.md の完全性チェック）
  - framework audit --quick 実行時

チェック項目（全て必須）:
────────────────────────────────────────────

  1. §3-E 入出力例（Example Table）
     □ セクションが存在するか（空でない）
     □ 5行以上のデータがあるか
     □ 正常系が2ケース以上あるか
     □ 異常系が3ケース以上あるか

  2. §3-F 境界値（Boundary Values）
     □ セクションが存在するか（空でない）
     □ §4.1 のデータ項目数と行数が一致するか
     □ 最小値/最大値/空/NULL/不正形式 の5列が埋まっているか

  3. §3-G 例外応答（Exception Response）
     □ セクションが存在するか（空でない）
     □ §9 のエラーケース数と行数が一致するか
     □ HTTPステータス/エラーコード/メッセージが埋まっているか

  4. §3-H 受け入れテスト（Gherkin）
     □ セクションが存在するか（空でない）
     □ Feature: と Scenario: が存在するか
     □ §3 の MUST 要件数と Scenario 数が対応するか

  5. 完全性チェックリスト
     □ SSOT 冒頭のチェックリストが存在するか
     □ §3-E/F/G/H の行が ✅ になっているか

判定:
────────────────────────────────────────────
  全項目 ✅ → Ready（実装開始可能）
  1つでも ☐ → Not Ready（実装開始禁止、不足箇所を報告）

Not Ready の場合の報告形式:
────────────────────────────────────────────
  「[機能ID] の Pre-Implementation Readiness Check が不合格です。

  不足セクション:
  - §3-F 境界値: §4.1 に5項目あるが §3-F は2行のみ（3項目不足）
  - §3-H Gherkin: MUST要件が4つあるが Scenario は2つのみ（2シナリオ不足）
...
| | Pre-Implementation Readiness Check（実装前軽量事前監査）追加。§3-E/F/G/H 全滅問題への構造的防止策 | |