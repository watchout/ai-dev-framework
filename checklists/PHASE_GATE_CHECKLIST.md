...
| 1.10 | 設計レビューを通過した | ☐ | |
| 1.11 | 🔒 §3-E 入出力例が5ケース以上（正常2+異常3）ある | ☐ | |
| 1.12 | 🔒 §3-F 境界値が全データ項目で定義されている | ☐ | |
| 1.13 | 🔒 §3-G 例外応答が全エラーケースで定義されている | ☐ | |
| 1.14 | 🔒 §3-H Gherkin テストが全MUST要件に存在する | ☐ | |
| 1.15 | 🔒 SSOT冒頭の完全性チェックリストが全項目 ✅ | ☐ | |

### L3レベル判定基準（12_SSOT_FORMAT.md 準拠）
...
| 2.6 | 技術レビューを通過した | ☐ | |
| 2.7 | 🔒 開発環境が起動する（docker-compose + pnpm dev） | ☐ | |
| 2.8 | 🔒 CI がグリーン（lint + type-check + test） | ☐ | |
| 2.9 | 🔒 全SSOTの §1 + §11 分析完了、Wave 分類済み | ☐ | |
| 2.10 | 🔒 GitHub Projects ボードが作成されている | ☐ | |
| 2.11 | 🔒 P0機能の親 Issue + Tasklist が作成されている | ☐ | |
| 2.12 | 🔒 ブランチ戦略が確認済み（main 直接コミット禁止） | ☐ | |
