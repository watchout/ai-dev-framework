...
export function generateSSOTStub(
  ...
  switch (expected.ssoId) {
    ...
    default:
      return `# ${expected.name}\n\n> Generated by shirube retrofit on ${date}\n> Project: ${projectName}\n\n---\n\n> **[要記入]** 既存の実装に基づいてこのドキュメントを記入してください。\n`;
  }
}
...
> Auto-detected by shirube retrofit on ${date}
...
2. Run \`shirube audit ssot <path>\` on each document
3. Use \`shirube status\` to track progress
`;
