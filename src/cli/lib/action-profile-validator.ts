...
const ROOT_FIELDS = ["profile_version", "product", "owner_repo", "surfaces"];
...
const NESTED_FIELDS: Record<string, string[]> = {
  resource_scope: ["tenant_scoped", "resource_patterns", "data_categories"],
  identity_requirements: [
    "actor_required",
    "agent_id_required",
    "human_user_required",
  ...
  rollback_policy: ["rollback_required", "rollback_kind", "replay_supported"],
  execution_policy: [
    "dry_run_supported",
...
export function validateActionProfiles(
  ...
  for (const document of documents) {
    ...
    validateRequiredFields(findings, mode, document.path, parsed, ROOT_FIELDS, "$");
...
function validateSurface(
  ...
  if (
    !Array.isArray(surface.capability_classes) ||
    surface.capability_classes.length === 0 ||
    !surface.capability_classes.every((item) => isEnumValue(item, CAPABILITY_CLASSES))
  ) {
    pushModeFinding(findings, mode, {
      path,
      surfaceId,
  ...
  for (const [field, requiredFields] of Object.entries(NESTED_FIELDS)) {
    validateNestedObject(
      findings,
      mode,
      path,
...
function validateBoundary(
  ...
  if (!isArrayOfStrings(boundary.interop_modes)) {
    pushModeFinding(findings, mode, {
      ...
      message: `interop_modes must be an array of: ${INTEROP_MODES.join(", ")}.`,
    });
  } else {
