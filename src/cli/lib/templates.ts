...
実装前に \`shirube gate check\`, \`shirube gate spec\`, \`shirube trace verify\` を確認する。
...
export function generateClaudeMd(config: ProjectConfig): string {
  ...
  if (config.profileType === "mcp-server") {
    return generateMcpServerClaudeMd(config, today);
  }
...
### Issue → タスク完了フロー
1. \\\`shirube plan --sync\\\` → 親Issue + タスクIssue が GitHub に作成される
2. 各タスクは DB → API → UI → Integration → Test → Review の順で実施
3. タスク完了時に \\\`shirube run\\\` が自動で GitHub Issue を close
...
Phase 5: 実装計画（shirube plan → Gate B 通過）
