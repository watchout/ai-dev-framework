...
export function registerConveyorCommand(program: Command): void {
  ...
  conveyor
    .command("plan")
    .description("Generate deterministic PR units/audit units, or a handoff/checklist/review-plan package for one queued Cell")
    .requiredOption("--cell-queue <path>", "shirube-cell-queue/v1 JSON file")
    .option("--cell-id <id>", "Cell id to plan")
    .option("--cell-ids <ids>", "Comma-separated Cell ids to consider for PR-unit batching")
      ...
      runConveyorAction(options, () => {
        ...
        if (!options.cellId) {
          const report = buildConveyorDeliveryPlan({
            cellQueue: readJsonFile(requiredCliOption(options.cellQueue, "--cell-queue")) as ConveyorCellQueue,
            cellIds: splitCsv(options.cellIds),
            generatedAt: options.generatedAt,
          });
          process.stdout.write(JSON.stringify(report, null, 2) + "\n");
          return;
  ...
  conveyor
    .command("open-pr")
    .description("Emit a draft PR plan for a queued Cell or PR unit; does not call GitHub or merge")
    .requiredOption("--cell-queue <path>", "shirube-cell-queue/v1 JSON file")
    .option("--cell-id <id>", "Cell id to open as draft PR")
    .option("--cell-ids <ids>", "Comma-separated Cell ids to consider for PR-unit batching")
    .option("--parent-ssot <owner/repo#issue>", "Parent SSOT override")
    .option("--repo <owner/repo>", "Repository override")
    .option("--framework-ref <ref>", "Pinned framework ref for Work Order preview")
    ...
    .action((options: ConveyorOpenPrOptions) => {
      runConveyorAction(options, () => {
        requireJsonFormat(options.format);
        if (!options.cellId) {
          const deliveryPlan = buildConveyorDeliveryPlan({
            cellQueue: readJsonFile(requiredCliOption(options.cellQueue, "--cell-queue")) as ConveyorCellQueue,
            cellIds: splitCsv(options.cellIds),
            generatedAt: options.generatedAt,
          });
          const firstUnit = deliveryPlan.pr_units[0] ?? null;
          process.stdout.write(JSON.stringify({
            schema: "shirube-conveyor-open-pr/v1",
            verdict: deliveryPlan.verdict,
            ...
            pr_unit_id: firstUnit?.pr_unit_id ?? null,
            covered_cells: firstUnit?.covered_cells ?? [],
            title: firstUnit ? `[Shirube PR Unit] ${firstUnit.pr_unit_id}` : null,
            body: firstUnit
              ? `Draft PR generated by Shirube Conveyor plan.\n\npr_unit_id: ${firstUnit.pr_unit_id}\ncovered_cells: ${firstUnit.covered_cells.join(",")}\naudit_unit_id: ${firstUnit.audit_unit_id}\n\nThis command does not synthesize audit, owner approval, or merge authority.`
              : null,
            next_action: deliveryPlan.next_action,
            blockers: deliveryPlan.blockers,
            warnings: deliveryPlan.warnings,
          }, null, 2) + "\n");
          return;
        }
