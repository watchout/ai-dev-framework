...
function completeWorkOrder(): Record<string, unknown> {
  return {
    ...
    github_state_ref: {
      ...
      pr_url: "https://github.com/watchout/ai-dev-framework/pull/244",
      durable_state: "GitHub issue/PR comments and labels are the SSOT.",
    },
    ...
    phase_goal: {
      ...
      scope: ["Work Order workflow check and docs."],
      non_scope: ["AUN live dispatch.", "merge automation."],
      acceptance_criteria: ["workflow check passes with complete fixture."],
      allowed_implementation_actions: ["edit files", "run checks", "open PR"],
      required_checks: ["workflow test", "type-check"],
      stop_conditions: ["protected execution requested"],
      evidence_writeback: ["PR comment with validation summary"],
      next_phase_handoff: "L1 implementation audit",
    },
    runner_policy: {
      policy: "codex_native_fast_lane",
      runner_agnostic_contract: true,
      execution_loop: "bounded_goal_mode",
      stop_lane_on: ["protected boundary", "approval required"],
    },
    evidence_contract: {
      required: ["PR comment", "CI/check evidence when triggered", "review link"],
      not_sufficient: ["AUN ACK", "queue row", "Discord projection"],
    ...
    acceptance_criteria: ["Complete Work Order fixture passes strict warning audit."],
    role_flow: ["arc", "adf-lead", "audit", "qa", "cto-if-required"],
    current_owner: "adf-lead",
    next_action: "open draft implementation PR",
    evidence_required: ["PR comment", "focused tests", "review links"],
    required_review: ["L1 implementation audit", "L2 protected behavior audit"],
