...
# オプション
shirube gate quality --phase early          # WARNING閾値を緩和（Wave 1-2）
shirube gate quality --sequential           # デバッグ用：順次実行
shirube gate quality --timeout 180          # タイムアウト延長
shirube gate quality --context-only         # コンテキスト収集のみ（手動実行用）
```
...
```bash
# 1. コンテキスト収集のみ
shirube gate quality --context-only
...
```bash
shirube gate quality --auto-fix               # デフォルト2回リトライ
shirube gate quality --auto-fix --max-retries 3  # 最大3回
```
